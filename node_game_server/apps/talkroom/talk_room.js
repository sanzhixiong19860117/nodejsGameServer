...
function broadcast_cmd(ctype, body, noto_user) {
	...
	for(var key in room) {
		...
		else if(proto_type == proto_mgr.PROTO_BUF) {
			if (buf_encoded == null) {
...
function on_user_enter_talkroom(session, body, utag, proto_type) {
	if (typeof(body.uname) == "undefined" || 
		...
		session.send_cmd(STYPE_TALKROOM, TalkCmd.Enter, Respones.INVALID_PARAMS, utag, proto_type);
		return;
	}
	if (room[utag]) { // 已经在聊天室
		session.send_cmd(STYPE_TALKROOM, TalkCmd.Enter, Respones.IS_IN_TALKROOM, utag, proto_type);
		return;
	}
	// 告诉我们的客户端，你进来成功了
	session.send_cmd(STYPE_TALKROOM, TalkCmd.Enter, Respones.OK, utag, proto_type);
	// end 
...
function on_user_exit_talkroom(session, is_lost_connect, utag, proto_type) {
	if (!room[utag]) { // 不再我的聊天室，你也没有离开这么一说
		if (!is_lost_connect) {
			session.send_cmd(STYPE_TALKROOM, TalkCmd.Exit, Respones.NOT_IN_TALKROOM, utag, proto_type);
		}
		return;
	}
	...
	// 发送命令, 你已经成功离开了。
	if (!is_lost_connect) {
		session.send_cmd(STYPE_TALKROOM, TalkCmd.Exit, Respones.OK, utag, proto_type);	
	}
}
